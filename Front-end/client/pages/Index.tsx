...
export default function Index() {
  ...
  const [activeTab, setActiveTab] = useState<"accommodations" | "myTrips">("accommodations");
  
  const { data: accommodationsData, isLoading, error } = useAccommodations();

  // Transform API response to match AccommodationCard props
  const accommodations = accommodationsData?.map((acc) => ({
    id: acc.id.toString(),
    images: acc.images && acc.images.length > 0 ? acc.images : (acc.main_image ? [acc.main_image] : []),
    title: acc.title,
    capacity: `${acc.capacity} نفر - بدون نفر اضافه`,
    rating: acc.rating, // Backend now returns as number
    location: `${acc.province} ، ${acc.city}`,
    pricePerNight: `${parseFloat(acc.price_per_night).toLocaleString('fa-IR')} تومان`,
  })) || [];
      ...
      <div className="w-full min-w-[320px] max-w-[550px] mx-auto bg-bg-secondary">
        ...
        <div className="flex items-center w-full h-14 px-4 py-2 pb-3">
          <div className="flex flex-row-reverse items-center justify-between w-full h-9">
            ...
            <button className="inline-flex items-center gap-2.5 p-2 rounded-xl bg-neutral-200">
        ...
        <div className="flex justify-center items-center gap-px rounded-xl bg-white mx-4 mt-3 h-12">
          <button
            ...
            className={`flex-1 flex flex-col justify-end items-center gap-3 py-3 px-11 ${
              activeTab === "accommodations" ? "pb-px" : ""
            }`}
          >
            <div 
              className="text-left tab-text-responsive"
              style={{
                color: activeTab === "accommodations" ? 'var(--Text-Secondary, #4F545E)' : 'var(--Text-Tertiary, #91969F)',
                fontFamily: 'IRANYekanXFaNum',
                fontStyle: 'normal',
                fontWeight: activeTab === "accommodations" ? 700 : 500
              }}
            >
              اقامتگاه‌ها
            </div>
          ...
          <button
            ...
            className={`flex-1 flex flex-col justify-end items-center gap-3 py-3 px-10 ${
              activeTab === "myTrips" ? "pb-px" : ""
            }`}
          >
            <div 
              className="text-left tab-text-responsive"
              style={{
                color: activeTab === "myTrips" ? 'var(--Text-Secondary, #4F545E)' : 'var(--Text-Tertiary, #91969F)',
                fontFamily: 'IRANYekanXFaNum',
                fontStyle: 'normal',
                fontWeight: activeTab === "myTrips" ? 700 : 500
              }}
            >
              سفرهای من
            </div>
          ...
          {activeTab === "accommodations" && (
            <div className="flex flex-col gap-2">
              ...
              {!isLoading && !error && accommodations.length === 0 && (
                <div className="flex items-center justify-center py-8">
                  <div className="text-text-tertiary text-sm">اقامتگاهی یافت نشد</div>
                </div>
              )}
              {!isLoading && !error && accommodations.map((accommodation) => (
